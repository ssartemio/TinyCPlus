...
#elif defined(__APPLE__) && defined(TC_GUI_COCOA)
    if (!window->headless && !tc_gui_cocoa_create(window, width, height, title, error)) {
        tc_gui_buffer_destroy(window->buffer);
        free(window);
        return NULL;
    }
...
#elif defined(__APPLE__) && defined(TC_GUI_COCOA)
    if (window && !window->headless)
        tc_gui_cocoa_next(window, timeout, &event);
    else
#endif
    if (window)
